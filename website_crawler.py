...
from scrapy_bridge import crawl_with_scrapy
        ...
        with sync_playwright() as p:
            ...
            while ((priority_queue or normal_queue)
                ...
                try:
                    ...
                    html = page.content()
                    final_url = page.url
                    print(f"[CRAWLER] Loaded: "
                          f"{final_url}")
                except PlaywrightTimeoutError:
                    print(f"[CRAWLER] Timeout: "
                         f"{current_url}")
                    visited.add(current_url)
                    continue
                except Exception as e:
                    print(f"[CRAWLER] Page error: "
                          f"{current_url}" )
                    print(f"{type(e).__name__}: {e}")
                    visited.add(current_url)
                    continue
...
def analyze_website(
    ...
    if scrapy_pages:
       print(
          f"[CRAWLER] Scrapy returned "
          f"{len(scrapy_pages)} pages"
        )
    else:
        print(
            "[CRAWLER] Scrapy returned no pages."
        )
    pages = scrapy_pages
    ...
    if linkedin_followers == "Not Found":
       missing_fields.append("LinkedIn Followers")

    print(
        "[SCRAPY] Missing fields: "
        + (
           ", ".join(missing_fields)
           if missing_fields
           else "None"
